...
from core.models import Word, Category, Example
...
class ExampleTestsViews(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(name='testcategory', is_created_by_user=False, created_by=self.user)
    ...
    def test_example_create(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(f'/api/example/{self.word.id}/', {
            'example': '私は見える',
            'reading': 'わたしはみえる',
            'meaning': 'I can see',
            'annotation': 'Teste',
    ...
    def test_example_create_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(f'/api/example/{9999}/', {
            'example': '私は見える',
            'reading': 'わたしはみえる',
            'meaning': 'I can see',
            'annotation': 'Teste',
    ...
    def test_example_get(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(f'/api/example_detail/{self.word.id}/{self.example.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_example_update(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/example_detail/{self.word.id}/{self.example.id}/', {
            'example': '私は見える'
        })
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_example_update_word_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/example_detail/{999}/{self.example.id}/', {
            'example': '私は見える'
        })
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_example_update_example_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/example_detail/{self.word.id}/{999}/', {
            'example': '私は見える'
        })
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_example_update_word_id_and_example_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/example_detail/{999}/{999}/', {
            'example': '私は見える'
        })
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_example_delete(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(f'/api/example_detail/{self.word.id}/{self.example.id}/')
