...
class TextWritingTestsViews(TestCase):
    ...
    def test_text_writing_create(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post('/api/text_writing/', {
            'title': '神のイタズラ',
            'text': '''
                    ある日、神様が、人間の世界を見ていました。
                    すると、人間たちが、いろいろなことで、争っているのが見えました。
    ...
    def test_text_writing_update(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_writing_detail/{self.text_writing.id}/', {
            'title': '神のイタズラ'
        })
    ...
    def test_text_writing_update_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_writing_detail/{999}/', {
            'title': '神のイタズラ'
        })
    ...
    def test_text_writing_delete(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(f'/api/text_writing_detail/{self.text_writing.id}/')
