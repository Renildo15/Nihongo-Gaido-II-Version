...
class TextTranslateTestsViews(TestCase):
    ...
    def test_text_translate_create(self):
        ...
        response = self.client.post(f'/api/text_translate/{self.text.id}/', {
            'title': 'Travessuras de Deus',
            'text': '''
                    Um dia, Deus estava olhando para o mundo dos humanos.
                    Então, ele viu os humanos brigando por várias coisas.
    ...
    def test_text_translate_create_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.post(f'/api/text_translate/{9999}/', {
            'title': 'Travessuras de Deus',
            'text': '''
                    Um dia, Deus estava olhando para o mundo dos humanos.
                    Então, ele viu os humanos brigando por várias coisas.
    ...
    def test_text_translate_update(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_translate_detail/{self.text.id}/{self.text_translate.id}/', 
                                     {'title': 'Travessuras de Deus'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_text_translate_update_text_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_translate_detail/{999}/{self.text_translate.id}/', 
                                     {'title': 'Travessuras de Deus'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_text_translate_update_translate_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_translate_detail/{self.text.id}/{999}/', 
                                     {'title': 'Travessuras de Deus'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_text_translate_update_both_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.patch(f'/api/text_translate_detail/{999}/{999}/', 
                                     {'title': 'Travessuras de Deus'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_text_translate_delete(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(f'/api/text_translate_detail/{self.text.id}/{self.text_translate.id}/')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    def test_text_translate_delete_text_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(f'/api/text_translate_detail/{999}/{self.text_translate.id}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_text_translate_delete_translate_id_not_found(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.delete(f'/api/text_translate_detail/{self.text.id}/{999}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
