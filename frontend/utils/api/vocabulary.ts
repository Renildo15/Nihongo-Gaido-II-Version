...
import { TypeWord, TypeLevel } from "./types"
...
export function useWords() {
    ...
    const { 
        ...
        isValidating, 
        mutate
    } = useSWR<WordReponse>("/api/words", fetcchSimple);
...
export async function updateWord(wordId: number, { word, reading, meaning, type, level, category }: IWordUpdate) {
        ...
        try {
            const res = await axios.patch<IWordReponse>(`/api/word/${wordId}`, { word, reading, meaning, type, level, category });
            return res.data.message
        } catch (error: any) {
            throw new Error(error.message)
        }
