...
export default function FilterByMonth(props: IFilterMonthProps) {
    ...
    const uniqueMonth = useMemo(() => grammars?.reduce((months: IGrammarFilterMonth[], grammar) => {
    ...
    function handleSelectMonth(monthDescription: string) {
        ...
        const selectedMonth = monthDescription === "" ? null : uniqueMonth ? uniqueMonth.find(m => m.description === monthDescription) ?? null : null
    ...
    return (
        ...
        >
            ...
            {uniqueMonth?.sort().map((month, index) => (
                <Select.Item label={month.description} value={month.description} key={index} />
            )) ?? []}
        </Select>
    )
}