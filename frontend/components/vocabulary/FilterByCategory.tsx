...
export default function FilterByCategory(props: IFilterByCategoryProps) {
    ...
    return (
        <Select
            isDisabled={!categories || categoriesError !== undefined || categoriesIsLoading || !words || wordsError !== undefined || wordsIsLoading}
            size={'md'}
            _light={{
                bg: 'white'
            }}
        ...
        >
            ...
            {categories?.map((category, index) => (
                <Select.Item label={category.name} value={category.name} key={index} />
            )) ?? []}
        </Select>
    )
}